...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--threshold_method", type=str, default="otsu", choices=['otsu', 'adaptive', 'adaptive_inversion'], help="Thresholding method.")
...
def apply_threshold(image: np.ndarray, method: str = 'otsu') -> np.ndarray:
    ...
    elif method == 'adaptive_inversion':
        binary_image = cv2.adaptiveThreshold(image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                             cv2.THRESH_BINARY_INV, 11, 2)
...
def morphological_operations(image: np.ndarray, kernel: int = 3) -> np.ndarray:
    ...
    morph_image = cv2.morphologyEx(morph_image, cv2.MORPH_OPEN, kernel_matrix)
    return morph_image
